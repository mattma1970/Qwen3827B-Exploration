...
class Game {
  constructor(ctx, size) {
    ...
    this.player = null;
    this.turkeys = [];
    this.state = "title"; // title | ready | play | dying | levelup | gameover
    this.stateT = 0;
    this.t = 0;
    this.paused = false;
    this.score = 0;
  ...
  loadLevel() {
    ...
    for (let r = 0; r < ROWS; r++) {
      for (let c = 0; c < COLS; c++) {
        ...
        if (ch === "M") { this.playerStart = { r, c }; this.grid[r][c] = " "; }
  ...
  updatePlay(dt) {
    ...
    if (this.frightT > 0) {
      this.frightT -= dt;
      if (this.frightT <= 0) { this.frightT = 0; this.chain = 0; }
    } else {
      this.cycleT += dt;
      let t = this.cycleT;
      let step = PHASE_DURATIONS.length - 1;
      for (let i = 0; i < PHASE_DURATIONS.length; i++) {
        if (t < PHASE_DURATIONS[i]) { step = i; break; }
        t -= PHASE_DURATIONS[i];
      }
  ...
  render() {
    ...
    if (this.state === "title") { this.renderTitle(c, S); return; }
  ...
  renderTurkeys(c) {
    for (let i = 0; i < this.turkeys.length; i++) {
      ...
      const facing = g.dir === "left" ? "left" : g.dir === "right" ? "right"
        : g.dir === "up" ? "right" : g.dir === "down" ? "right" : "right";
